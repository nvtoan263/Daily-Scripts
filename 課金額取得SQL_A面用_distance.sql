select se.segment_id, ta.*, ti.day_class_id, ti.time_class_id, CAST(ab.start_time AS INT) start_time, CAST(ab.end_time AS INT) end_time, lo.location_class_id  
from erp_tariff_class_A ta, time_classes_A ti, location_classes_A lo, segment_A se, absolute_time_of_day_A ab   
...
and CAST(ta.table_type AS INT) = 149   
and CAST(ti.table_type AS INT) = 149   
and CAST(lo.table_type AS INT) = 149   
and CAST(lo.location_class_id AS INT) = CAST(se.location_class AS INT)
and CAST(ti.time_class_id AS INT) = CAST(ab.time_class_id AS INT)
and CAST(ab.table_type AS INT) = 137
and (  
	(
		(
...
and CAST(se.segment_id AS INT) in (
